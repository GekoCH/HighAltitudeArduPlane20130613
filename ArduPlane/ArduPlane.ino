	  ...
	  */
	  ...
	  //mit fernsteuerung kann ich mehr pitch gegen oder nicht, aber nur immer gegen unten also negativer pitch
		...
		if (failsafeCounter <= 0){
			...
			if (hal.rcin->read(CH_7) > 1550 && current_loc.alt >= 300000UL){ //0.1*B5-155
				nav_pitch_cd = oldPitchit + ((abs(hal.rcin->read(CH_7) - 1550) * 0.1) - 155) * 100.0;
				ChangePitch = 1;
			}else if(hal.rcin->read(CH_7) < 1450 && current_loc.alt >= 300000UL){
				nav_pitch_cd = oldPitchit - ((abs(hal.rcin->read(CH_7) - 1450) * 0.1) - 155) * 100.0;
				ChangePitch = 1;
			}else{
				ChangePitch = 0;
			}
